...
data = getText("pic/CogsNotes.docx")
...
top3_words = []
for i in range(cl):
	temp_str = str(getStrings(df_results[df_results["label"] == i]["results"]))
	top30_strings.append(temp_str)
	...
	top30_dict.append(cnt.most_common(10))
	#print(top30_dict[i])

	words = []
	for j in range(3):
		words.append(top30_dict[i][j+1][0])
	top3_words.append(words)

print(top3_words)

total_list = []
for strings in top3_words:
	data = {}
	data["one"] = strings[0]
	data["two"] = strings[1]
	data["three"] = strings[2]
	total_list.append(data)
 
with open("words.json", 'w', encoding="utf8") as outfile:
	outfile.write("words = '")
	json.dump(total_list, outfile)
	outfile.write("';")


total_str = ""
for i in range(cl):
	str1 = ''.join(top30_strings[i])
	total_str += str1

#print(total_str)

list_ordered = [df_results[df_results['label'] == i] for i in range(cl)]
print(list_ordered)
